...
class e1_bonus_stream extends HandsOnSuite {
  ...
  final class Cons[A](val head: A, tl: => Stream[A]) extends Stream[A] {
    ...
    def map[B](fonction: A => B): Stream[B] =
      new Cons(fonction(head), tail.map(fonction))
    ...
    override def filter(fonction: A => Boolean): Stream[A] =
      if (fonction(head)) Cons(head, tail.filter(fonction))
      else tl.filter(fonction)

    override def equals(that: Any): Boolean = that match {
      case Cons(h, t) => head.equals(h) && tl.equals(t)
      case Nil        => false
    }
  ...
  object Cons {
    ...
    def unapply[A](cons: Cons[A]): Option[(A, Stream[A])] = Stream.unapply(cons)
  }
  ...
  case object EmptyStream extends Stream[Nothing] {
    ...
    def map[B](fonction: A => B): Stream[B] = this

    def flatMap[B](fonction: A => Stream[B]): Stream[B] = this

    def filter(fonction: A => Boolean): Stream[A] = this
  ...
  exercice("union") {
    ...
    Stream(1, 2, 3).union(Stream("A", "B", "C")) should be(
      Stream(1, 2, 3, "A", "B", "C"))
  ...
  exercice("flatMap") {
    ...
    val combinaison = for (a <- Stream("A", "B"); i <- Stream(1, 2))
      yield (a + i)
