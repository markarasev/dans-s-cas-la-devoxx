...
class e0_une_mise_en_abime /* ou un sac de sac */ extends HandsOnSuite {
  ...
  case class Sac(valeur: Int, items: Set[String] = Set("un pc")) {
    ...
    /**
      ...
      * @return un Sac
      */
    def map(fonction: Int => Int): Sac = copy(fonction(valeur))
    ...
    def flatMap(fonction: Int => Sac): Sac = {
      ...
      res.copy(items = items ++ res.items)
    }
  }

  exercice(
    "Je peux créer mon sac avec un seul entier, et faire des choses avec mon sac") {
    ...
    incrémenteUnSac(monPetitSacDeZero).items contains "un portable" should be(
      true)
  ...
  exercice("je peux appliquer une expression en for sur mon sac") {
    ...
    val monPetitSacDeUn =
      for (i <- monPetitSacDeZero) yield i + 1
  ...
  exercice("je peux appliquer une expression imbriquée dans mes Sac") {
    ...
    val l_union_de_mes_sac = for (p <- monPetitSacDeDeux; g <- monGrosSacDeCent)
      yield p * g
