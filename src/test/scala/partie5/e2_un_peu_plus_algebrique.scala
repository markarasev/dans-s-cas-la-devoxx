...
class e2_un_peu_plus_algebrique extends HandsOnSuite {
  ...
  object Sac {
    def apply(valeur: Int, items: Set[String] = Set.empty): Sac =
      SacPlein(valeur, items)
  }

  case class SacVide(items: Set[String] = Set.empty) extends Sac {

    override def map(fonction: Int => Int): Sac = this

    override def flatMap(fonction: Int => Sac): Sac = this

    override def filter(fonction: Int => Boolean): Sac = this

    override def valeurOuSinon(replacement: Int): Int = replacement

    def addItems(items: Set[String]): Sac = SacPlein(0, items)
  }

  case class SacPlein(valeur: Int, items: Set[String] = Set.empty) extends Sac {

    override def map(fonction: Int => Int): Sac = Sac(fonction(valeur), items)

    override def flatMap(fonction: Int => Sac): Sac = fonction(valeur) match {
      case vide: SacVide  => this
      case SacPlein(v, i) => copy(v, items ++ i)
    }

    override def filter(fonction: Int => Boolean): Sac =
      if (fonction(valeur)) this
      else SacVide()

    override def valeurOuSinon(replacement: Int): Int = valeur

    def addItems(newItems: Set[String]): Sac = copy(items = items ++ newItems)
  }
  ...
  exercice("toujours comme avant, je peux combiner mes sac") {
    ...
    val combinaison = for (deux <- sacDeDeux; cent <- sacDeCent)
      yield (deux * cent)
    ...
    val sacVide = SacVide(Set("sac vide"))
    val combinaisonVide = for (deux <- sacVide; cent <- sacDeCent)
      yield (deux * cent)
