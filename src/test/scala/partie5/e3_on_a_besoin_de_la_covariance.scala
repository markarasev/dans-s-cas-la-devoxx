...
class e3_on_a_besoin_de_la_covariance extends HandsOnSuite {
  ...
  object Sac {
    def apply[A](valeur: A, items: Set[String] = Set.empty): Sac[A] =
  ...
  case class SacVide(items: Set[String] = Set.empty) extends Sac[Nothing] {
    ...
    override def map[B](fonction: A => B): Sac[B] = this

    override def flatMap[B](fonction: A => Sac[B]): Sac[B] = this

    override def filter(fonction: A => Boolean): Sac[A] = this

    override def valeurOuSinon[B >: A](replacement: B): B = replacement

    override val isEmpty: Boolean = true

    def addItems(items: Set[String]): Sac[A] = SacVide(items)

  }

  case class SacPlein[A](valeur: A, items: Set[String] = Set.empty)
      extends Sac[A] {

    override def map[B](fonction: A => B): Sac[B] =
      SacPlein(fonction(valeur), items)

    override def flatMap[B](fonction: A => Sac[B]): Sac[B] =
      fonction(valeur) match {
        case vide: SacVide  => vide
        case SacPlein(v, i) => SacPlein(v, items ++ i)
      }
    ...
    def addItems(i: Set[String]): Sac[A] = SacPlein(valeur, items ++ i)
