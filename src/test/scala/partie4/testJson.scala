...
object Implicits {
  ...
  implicit def seqWriter[B](implicit writer: Writer[B]) = Writer {
    seq: Seq[B] =>
      JsSeq(seq.map(writer.write))
  }
}
...
class testJson extends HandsOnSuite {
  exercice("a Json tree should print itself") {
    ...
    val jsonObject = JsObject(
      Map("string" -> jsonString, "number" -> jsonNumber, "seq" -> jsonSequece))
  ...
  exercice("toJson should correctly convert a BigDecimal") {
    val expected = JsNumber(BigDecimal(
      "99999999999999999999999999999999999999999999999999999999999999999999999999"))
    val actual = Json.toJson(BigDecimal(
      "99999999999999999999999999999999999999999999999999999999999999999999999999"))
  ...
  class testJsonClient extends HandsOnSuite {
    exercice("toJson should correctly convert a user") {
      ...
      val expected = JsObject(
        Map("name" -> JsString("Mathieu"),
            "age" -> JsNumber(25),
            "friends" -> JsSeq(
              Seq(JsString("Jean"), JsString("Jon"), JsString("Ludwine")))))
